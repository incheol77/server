                                     ...
                                     uint *bufsz, uint *flags, Cost_estimate *cost)
{
  ...
  /* Counter of blocks that contain range edges */
  ulonglong edge_blocks_cnt= 0;
  /* Counter of blocks that contain index tuples from ranges */
  ...
  */
  ...
  /*
    The counter of of single point ranges that we failed to assign
    to any block
  */
  ...
  uint avg_block_records= ((uint) (table->file->stats.block_size*0.75))/len + 1;
  ulonglong max_blocks= max_rows / avg_block_records + 1;
  ...
  *bufsz= 0;
  ...
  while (!seq->next(seq_it, &range))
  {
    ...
    if ((range.range_flag & UNIQUE_RANGE) && !(range.range_flag & NULL_RANGE))
    {
      ...
      */
      single_point_ranges++;
      edge_blocks_cnt++;                        // Not needed, for completeness
      range_blocks_cnt++;                       // Not needed, for completeness
    }
	...
	*/
	...
	{
          /*
            The new range starts not in the same block that the last range
            for which the position of the range was provided.
  ...
  ulonglong io_blocks= (range_blocks_cnt - edge_blocks_cnt +
                        (single_point_ranges - unassigned_single_point_ranges));
  set_if_smaller(io_blocks, max_blocks);
